...
@dataclass
class RealmanControllerConfig(BaseControllerConfig):
    """
...
class RealmanController(BaseController):
    config: RealmanControllerConfig

    def __init__(self, config: RealmanControllerConfig):
        super().__init__(config)

    ################## cls methods ##################
    def rm_set_hand_follow_pos(self, target_hand_pos: np.ndarray):
        """
        Args:
            target_hand_pos: np.ndarray, shape (hand_dof, ), target hand angles
                ro_hand has hand_dof=6 joints
        """
        target_hand_pos = np.clip(target_hand_pos, self.config.hand_pos_range[0], self.config.hand_pos_range[1])  # clip to valid range

        target_hand_pos = target_hand_pos.astype(np.int64)

        assert self.robot.rm_set_hand_follow_pos(target_hand_pos, 0) == 0, f"Failed to set hand follow position: {target_hand_pos}"

    def rm_set_hand_follow_angle(self, target_hand_angle: np.ndarray):
        """
        Args:
            target_hand_angle: np.ndarray, shape (hand_dof, ), target hand angles
                ro_hand has hand_dof=6 joints
        """
        target_hand_angle = np.clip(target_hand_angle, self.config.hand_angle_range[0], self.config.hand_angle_range[1])  # clip to valid range

        target_hand_angle = target_hand_angle.astype(np.int64)

        assert self.robot.rm_set_hand_follow_angle(target_hand_angle, 0) == 0, f"Failed to set hand follow angle: {target_hand_angle}"
    ...
    def _process_commands(self):
        ...
        if hasattr(self, 'target_hand_angle'):
            self.rm_set_hand_follow_angle(self.target_hand_angle)
        ...
        if hasattr(self, 'target_hand_pos'):
            self.rm_set_hand_follow_pos(self.target_hand_pos)
