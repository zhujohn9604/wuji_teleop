...
def pose_distance(start_pose, end_pose):
    ...
    end_pos = end_pose[:3]
    start_rot = st.Rotation.from_euler('xyz', start_pose[3:], degrees=False)
    end_rot = st.Rotation.from_euler('xyz', end_pose[3:], degrees=False)
    pos_dist = np.linalg.norm(end_pos - start_pos)
    rot_dist = rotation_distance(start_rot, end_rot)
    return pos_dist, rot_dist

class PosEulerTrajectoryInterpolator:
    """
    pos euler, in radians
    """
    def __init__(self, times: np.ndarray, poses: np.ndarray):
        ...
        if len(times) == 1:
            ...
            pos = poses[:, :3]
            rot = st.Rotation.from_euler('xyz', poses[:, 3:], degrees=False)
    ...
    def poses(self) -> np.ndarray:
        if self.single_step:
            ...
            poses[:,3:] = self.rot_interp(self.times).as_euler('xyz', degrees=False)
            return poses

    def trim(self,
            start_t: float, end_t: float
            ) -> "PosEulerTrajectoryInterpolator":
        ...
        return PosEulerTrajectoryInterpolator(times=all_times, poses=all_poses)
    ...
    def drive_to_waypoint(self,
        ...
        final_interp = PosEulerTrajectoryInterpolator(times, poses)
        return final_interp
    ...
    def __call__(self, t: Union[numbers.Number, np.ndarray]) -> np.ndarray:
        ...
        if self.single_step:
            ...
            pose[:,3:] = self.rot_interp(t).as_euler('xyz', degrees=False)
