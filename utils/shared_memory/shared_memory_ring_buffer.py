...
class SharedMemoryRingBuffer:
    ...
    def get(self, out=None, debug=False) -> Dict[str, np.ndarray]:
        if out is None:
        ...
        # 执行核心操作
        if debug:
            core_start_time = time.monotonic()
        count = self.counter.load()
        curr_idx = (count - 1) % self.buffer_size
        if debug:
            core_end_time = time.monotonic()

        # 预先获取所有数组引用
        if debug:
            arrays_start_time = time.monotonic()
        arrays = {}
        for key, value in self.shared_arrays.items():
            arrays[key] = value.get()
        if debug:
            arrays_end_time = time.monotonic()
        ...
        # 只在需要时打印调试信息
        if debug:
            print(f"总执行时间: {dt:.4f} vs 预算: {self.get_time_budget:.4f}, core time: {core_end_time - core_start_time:.4f}, arrays time: {arrays_end_time - arrays_start_time:.4f}, copy time: {copy_end_time - copy_start_time:.4f}")
