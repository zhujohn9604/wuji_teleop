...
def open(
    ...
    Returns:
        ...
        None if the device could not be opened
    """
    assert DeviceNumber > 0, f"DeviceNumber must be greater than 0, but is {DeviceNumber}"
    ...
    if not found_devices:
        ...
        # select first every 14 devs
        found_devices = found_devices[::14]

        assert len(found_devices) >= DeviceNumber, f"Found {len(found_devices)} devices, but DeviceNumber={DeviceNumber}"
