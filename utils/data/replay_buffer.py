...
from utils.torch_utils import device_get, device_put
...
class ReplayBuffer(Dataset):
    def __init__(
        ...
        include_mc_target_q: Optional[bool] = False,
    ):
        ...
        if include_mc_target_q:
            dataset_dict['mc_target_q'] = np.empty((capacity,), dtype=np.float32)
    ...
    def get_iterator(self, queue_size: int = 2, sample_args: dict = {}, device=None):
        ...
        def enqueue(n):
            for _ in range(n):
                ...
                queue.append(device_put(data, device=device))
