...
class RobotiqGripper:
    ...
    def __init__(
        ...
        self._max_position = calib_max_pos
        self._min_speed = 0
        self._max_speed = 255
        self._min_force = 0
        self._max_force = 255
    ...
    def move_to(self, open_ratio: float, speed: int, force: int):
        # clip to [0, 1]
        open_ratio = max(0.0, min(1.0, open_ratio))
        min_pos = self.get_min_position()
        max_pos = self.get_max_position()
        position = int(min_pos + open_ratio * (max_pos - min_pos))
        return self.move(position, speed, force)
