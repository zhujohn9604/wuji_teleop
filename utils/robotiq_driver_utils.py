...
class RobotiqDriver:
    ...
    def __init__(
        ...
        calibrate_on_activation: bool = True,
        ...
        self._max_position = 220    # # 255 is the maximum, since we use the new figure tip, we can only reach 220
        ...
        self.calibrate_on_activation = calibrate_on_activation
    ...
    def __enter__(self):
        self.connect()
        self.activate(self.calibrate_on_activation)
        return self
    ...
    def is_active(self):
        ...
        return RobotiqDriver.GripperStatus(status) == RobotiqDriver.GripperStatus.ACTIVE
    ...
    def auto_calibrate(self, log: bool = True) -> None:
        ...
        if RobotiqDriver.ObjectStatus(status) != RobotiqDriver.ObjectStatus.AT_DEST:
    ...
    def move_and_wait_for_recv(self, position: int, speed: int, force: int) -> Tuple[int, ObjectStatus]:  # noqa
        """Sends commands to start moving towards the given position, with the specified speed and force, and
        then waits for the command to be received.
        :param position: Position to move to [min_position, max_position]
        :param speed: Speed to move at [min_speed, max_speed]
        :param force: Force to use [min_force, max_force]
        ...
        """
        set_ok, _ = self.move(position, speed, force)
        if not set_ok:
            raise RuntimeError("Failed to set variables for move.")

        # report the actual position and the object status
        final_pos = self._get_var(self.POS)
        final_obj = self._get_var(self.OBJ)
        return final_pos, RobotiqDriver.ObjectStatus(final_obj)
    ...
    def move_and_wait_for_pos(self, position: int, speed: int, force: int) -> Tuple[int, ObjectStatus]:  # noqa
        ...
        while RobotiqDriver.ObjectStatus(cur_obj) == RobotiqDriver.ObjectStatus.MOVING:
            cur_obj = self._get_var(self.OBJ)
